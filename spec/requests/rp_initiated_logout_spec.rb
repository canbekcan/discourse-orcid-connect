...
describe "ORCID RP-Initiated Logout" do
  let(:document_url) do
    SiteSetting.orcid_connect_discovery_document =
  ...
  before do
    SiteSetting.orcid_connect_enabled = true
    SiteSetting.orcid_connect_rp_initiated_logout = true
  ...
  context "with user and token" do
    ...
    it "includes the redirect URI if set" do
      SiteSetting.orcid_connect_rp_initiated_logout_redirect = "https://example.com"
      delete "/session/#{user.username}", xhr: true
      expect(response.status).to eq(200)
      expect(response.parsed_body["redirect_url"]).to eq(
    ...
    it "does not redirect if plugin disabled" do
      SiteSetting.orcid_connect_enabled = false
      delete "/session/#{user.username}", xhr: true
      expect(response.status).to eq(200)
      expect(response.parsed_body["redirect_url"]).to eq("/")
    end
    ...
    it "does not redirect if rp initiated logout disabled" do
      SiteSetting.orcid_connect_rp_initiated_logout = false
      delete "/session/#{user.username}", xhr: true
      expect(response.status).to eq(200)
      expect(response.parsed_body["redirect_url"]).to eq("/")
    end
    ...
    context "with client_id included in logout endpoint" do
      before do
        SiteSetting.orcid_connect_client_id = "test-client-id"
        SiteSetting.orcid_connect_rp_initiated_logout_include_client_id = true
      end
