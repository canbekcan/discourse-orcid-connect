...
describe OrcIDConnectAuthenticator do
  ...
  context "when match_by_email is false" do
    it "does not match the user" do
      SiteSetting.orcid_connect_match_by_email = false
      result = authenticator.after_authenticate(hash)
      expect(result.user).to eq(nil)
    end
  end
  ...
  describe "group syncing" do
    context "when orcid_connect_groups_claim is blank" do
    ...
    context "when orcid_connect_groups_claim is set" do
      before { SiteSetting.orcid_connect_groups_claim = "groups" }
    ...
    context "with a custom claim name" do
      before { SiteSetting.orcid_connect_groups_claim = "cognito:groups" }
  ...
  describe "user field syncing" do
    ...
    context "with a mapping configured" do
      before do
        SiteSetting.orcid_connect_user_field_mappings = [
